...
5. Any edit to `~/.openclaw/openclaw.json` must follow constitution safety flow: backup → minimal diff → `jq` validate → minimal functional test → restart → rollback on failure.
