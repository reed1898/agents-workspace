...
## 12) 长耗时任务执行规则（强制）
1. 若任务预计耗时超过 1 分钟，或明显属于大任务（如写程序、下载/安装/配置环境、批量处理），必须优先下发到 sub-engine/subagent 执行。
2. 主 engine 只负责：任务编排、中间进度汇报、结果汇总，不得长时间阻塞主对话。
3. 在等待子任务期间，主会话必须保持可交互，避免“卡死聊天流程”。
4. 执行中需按阶段回报状态（开始/关键里程碑/完成或阻塞原因）。
5. 例外：预计 1 分钟内可完成的简单操作，可在主会话直接完成。
...
## 19) Cron / HeartBeat 统一交付规则（2026-03-04，强制）
1. 所有 Cron 和 HeartBeat 中执行的任务，其结果/汇报必须通过 Telegram Bot 私聊发送给 Reed。
2. 所有 Cron 和 HeartBeat 任务结果必须同步保存到知识库（`private/<agent>/...`），不得只发消息不落库。
