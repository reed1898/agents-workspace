...
if [[ -z "$APP_ID" || -z "$ACCESS_TOKEN" ]]; then
  ...
  if [[ -f "$CFG" ]] && command -v jq >/dev/null 2>&1; then
    [[ -z "$APP_ID" ]] && APP_ID="$(jq -r '.skills.entries["volcengine-stt"].appId // empty' "$CFG")"
    [[ -z "$ACCESS_TOKEN" ]] && ACCESS_TOKEN="$(jq -r '.skills.entries["volcengine-stt"].accessToken // empty' "$CFG")"
    CFG_RES="$(jq -r '.skills.entries["volcengine-stt"].resourceId // empty' "$CFG")"
...
REQ_ID="$(cat /proc/sys/kernel/random/uuid 2>/dev/null || uuidgen 2>/dev/null || echo "$(date +%s)-$RANDOM-$RANDOM")"
