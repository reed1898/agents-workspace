...
def get_sina_symbol(code: str) -> str:
    ...
    # 科创50: 000688 -> s_sh000688 (注意：000688在国城矿业，需要特殊处理)
    if code == "000001":  # 上证指数
        return "s_sh000001"
    elif code == "000688":  # 科创50 (注意：与国城矿业代码冲突，需区分)
        return "s_sh000688"
...
def fetch_realtime_sina(symbols: list[str]) -> dict[str, dict]:
    ...
    try:
        ...
        for line in text.strip().split("\n"):
            ...
            # 指数数据格式：名称,当前点数,涨跌点数,涨跌百分比,成交量(手),成交额(元)
            # 普通股票格式：名称,今开,昨收,现价,最高,最低,...(共32+字段)
            is_index = symbol.startswith(("s_sh", "s_sz"))
            ...
            if is_index:
                ...
                volume = int(float(fields[4])) if len(fields) > 4 and fields[4] else 0  # 手
                amount = float(fields[5]) if len(fields) > 5 and fields[5] else 0  # 元
                
                # 推算昨收和今开
                pre_close = price - change_amt if price else None
                open_price = pre_close  # 指数无法直接获取今开，用昨收代替
                high = price  # 无法直接获取最高最低
                low = price
            else:
                # 普通股票数据解析
                if len(fields) < 32:
